...
We can take the dataset that we just created, and with only a small modification look at how the behavior changes between these two categories. What we need to do is identify which days of the week each day is and then flag those that are weekend and weekday days respectively. Due to the nature of the plot we are going to use the lattice package.
